...
const blockColorMap: Record<string, string> = {
  ...
  documents: "text-muted-foreground",
...
function ContextBlock({ block }: { block: ContextBlock }) {
  ...
  if (isShort) {
    return (
      <div className="flex items-center gap-2 px-3 py-2 rounded-md border border-border bg-card">
        ...
        <span className="text-xs text-muted-foreground ml-auto font-mono truncate max-w-[60%] text-right">
          {block.content}
        </span>
      </div>
    );
        ...
        <ScrollArea className="flex-1 -mx-6 px-6">
          <div className="space-y-1.5 py-2">
            {contextMutation.isPending && !blocks ? (
              ...
              blocks.map((block) => (
                <ContextBlock key={block.key} block={block} />
              ))
            ) : (
              <div className="text-center py-8">
